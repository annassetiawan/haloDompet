    ...
    <section className="relative py-24 bg-zinc-950">
      <div className="container mx-auto px-4 lg:px-8">
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="border border-zinc-800/50 rounded-xl bg-zinc-900/30 px-6 data-[state=open]:bg-zinc-900/50 transition-colors duration-200"
              >
                <AccordionTrigger className="text-left hover:text-purple-400 transition-colors py-5 text-white font-medium">
                  {faq.question}
                </AccordionTrigger>
        ...
        <div className="text-center mt-16">
          <p className="text-zinc-400 mb-4">Punya pertanyaan lain?</p>
          <a
